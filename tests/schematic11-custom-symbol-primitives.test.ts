...
test.each([
  ...
  async (_label, strokeWidth) => {
    ...
    expect(circle?.getNumber("RADIUS")).toBe(100)
    expect(circle?.getNumber("SECONDARYRADIUS")).toBe(100)
    ...
    expect(ellipticalArc?.getNumber("RADIUS")).toBe(133)
    expect(ellipticalArc?.getNumber("SECONDARYRADIUS")).toBe(66)
