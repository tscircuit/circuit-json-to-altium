...
test("round-trips the open-source SimpleFOC StepMini Altium board", async () => {
  const result = await createOpenSourceBoardRoundTrip({
    boardName: "SimpleFOC StepMini",
  ...
  expect(result.sourcePrimitiveTotal).toBeGreaterThan(750)
  await expect(
