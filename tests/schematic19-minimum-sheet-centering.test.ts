...
test("uses the matching source geometry for each schematic sheet", () => {
  const elements: CircuitElement[] = [
    board(),
    {
    ...
    schematicSheets: [
      {
        ...
        circuitOrigin: { x: 1, y: 2 },
        height: 40,
        width: 50,
      },
      {
        schematicSheetId: "sheet-b",
        circuitOrigin: { x: 3, y: 4 },
        height: 70,
        width: 90,
      },
    ],
  ...
  const childSchematics = converter
    .getOutput()
    .schematics.slice(1)
    .map((schematic) => parseAltiumSchDoc(schematic.content))
