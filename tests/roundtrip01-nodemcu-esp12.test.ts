...
test("round-trips the open-source NodeMCU ESP-12 Altium board", async () => {
  ...
  expect(result.sourceNativeArcCount).toBe(251)
  expect(result.roundTripNativeArcCount).toBe(result.sourceNativeArcCount)
  expect(result.arcBulgeMismatchCount).toBe(0)
  expect(result.arcGeometryMismatches).toEqual([])
  expect(result.arcGeometryMismatchCount).toBe(0)
  expect(result.arcRadiusMismatchCount).toBe(0)
